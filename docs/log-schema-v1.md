...
* **`vector.log.schema`** — назва схеми подій
  Поточне значення: **`adheart`** — саме за цим значенням Vector pipeline маршрутизує івент через JSON-валідатор schema v1 і витягує `service.*` з тіла повідомлення у Loki labels. Якщо лейбл не задано, Vector підставляє `adheart` за замовчуванням, тому для adheart-сервісів його можна не вказувати, але рекомендуємо ставити явно. Закладено можливість розширення (наприклад, **`otel`**), якщо у майбутньому перейдемо на відповідну модель.
